-- ============================================================================
-- EliteCommentCraft Database Schema - Fixed Order
...
-- ============================================================================
-- SECURITY FUNCTIONS (created after tables exist)
-- ============================================================================
...
RETURNS TABLE(id UUID, email TEXT, role ec_user_role, created_at TIMESTAMPTZ, updated_at TIMESTAMPTZ, last_login_at TIMESTAMPTZ, display_name TEXT)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
...
BEGIN
    ...
    SELECT up.role INTO old_role FROM ec_user_profiles up WHERE up.id = _target_user_id;
    ...
    -- Update the user role and return the updated profile
    UPDATE ec_user_profiles 
    SET role = _new_role, updated_at = NOW()
    WHERE ec_user_profiles.id = _target_user_id;
    ...
    RETURN QUERY
    SELECT up.id, up.email, up.role, up.created_at, up.updated_at, up.last_login_at, up.display_name
    FROM ec_user_profiles up
    WHERE up.id = _target_user_id;
END;
$$;
...
BEGIN
    ...
    SELECT up.id INTO target_user_id FROM ec_user_profiles up WHERE up.email = _admin_email;
    ...
    WHERE ec_user_profiles.id = target_user_id;
