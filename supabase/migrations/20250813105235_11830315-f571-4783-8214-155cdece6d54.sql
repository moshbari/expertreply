-- ============================================================================
-- EliteCommentCraft Database Schema - Simplified Types
...
CREATE OR REPLACE FUNCTION ec_admin_set_role(_target_user_id UUID, _new_role TEXT, _reason TEXT DEFAULT NULL)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
    ...
    IF _new_role NOT IN ('admin', 'user', 'interested') THEN
        RAISE EXCEPTION 'Invalid role. Must be admin, user, or interested.' USING ERRCODE = '22023';
    END IF;

    -- Get the current role
    SELECT role::TEXT INTO old_role FROM ec_user_profiles WHERE id = _target_user_id;
    ...
    SET role = _new_role::ec_user_role, updated_at = NOW()
...
BEGIN
    ...
    SELECT id INTO target_user_id FROM ec_user_profiles WHERE email = _admin_email;
    ...
    INSERT INTO ec_role_changes (target_user_id, changed_by_user_id, old_role, new_role, reason)
    VALUES (target_user_id, target_user_id, 'interested', 'admin', 'Bootstrap admin');
...
CREATE POLICY "Users can update their own profile display name"
...
CREATE POLICY "Users and admins can create analysis requests"
...
CREATE POLICY "Users and admins can create comment generations"
...
COMMENT ON FUNCTION ec_admin_set_role(UUID, TEXT, TEXT) IS 'Admin function to change user roles with audit logging';
